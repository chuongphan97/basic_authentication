...
public class SwaggerConfig implements WebMvcConfigurer {
    ...
    public Docket apiUser() {
        ...
        return new Docket(DocumentationType.SWAGGER_2).groupName("User API").select()
                .apis(RequestHandlerSelectors.basePackage("com.example.authentication.api.user"))
    ...
    public Docket apiAdmin() {
        ...
        return new Docket(DocumentationType.SWAGGER_2).groupName("Admin API").select()
                .apis(RequestHandlerSelectors.basePackage("com.example.authentication.api.admin"))
                .paths(Predicates.not(PathSelectors.regex(Constants.PATH_SELECTOR_REGEX_ERROR)::apply)).build()
                .genericModelSubstitutes(APIResponse.class).
                alternateTypeRules(
                        newRule(resolver.resolve(DeferredResult.class,
                                        resolver.resolve(APIResponse.class, WildcardType.class)),
    ...
    private ApiInfo metaData() {
        return new ApiInfoBuilder().version("1.0").title("DEMO AUTHENTICATION API")
                .description("Documentation for Demo authentication").build();
    }
